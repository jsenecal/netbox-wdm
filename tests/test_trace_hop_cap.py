...
max_trace_hops plugin setting, counted in cable segments, and hitting it
logs a warning.
"""
...
# Each pass-through unit costs three cable segments, plus three to leave the
# chain, so eight units sit comfortably between the two caps exercised here.
CHAIN_UNITS = 8
CAP_BELOW_CHAIN = 10
CAP_ABOVE_CHAIN = 50
...
class TestDiscoveryHopCap:
    ...
    def test_long_chain_resolves_with_raised_cap(self, wdm_site, dt_cwdm_sf, dt_pp, wdm_roles):
        """A chain longer than the cap resolves once max_trace_hops is raised (issue #43)."""
        topo = sf_mux_long_chain(wdm_site, dt_cwdm_sf, dt_pp, wdm_roles, passthrough_units=CHAIN_UNITS)
        start_rp = topo.bundles["mux_a"].line_ports["bidi"].rear_port
        with override_settings(PLUGINS_CONFIG=_plugins_config(CAP_ABOVE_CHAIN)):
    ...
    def test_cap_hit_logs_warning(self, wdm_site, dt_cwdm_sf, dt_pp, wdm_roles, caplog):
        """Hitting the cap truncates discovery and logs a warning (issue #43)."""
        topo = sf_mux_long_chain(wdm_site, dt_cwdm_sf, dt_pp, wdm_roles, passthrough_units=CHAIN_UNITS)
        start_rp = topo.bundles["mux_a"].line_ports["bidi"].rear_port
        with override_settings(PLUGINS_CONFIG=_plugins_config(CAP_BELOW_CHAIN)):
...
class TestCableSegmentHopCap:
    ...
    def test_long_chain_traces_with_raised_cap(self, wdm_site, dt_cwdm_sf, dt_pp, wdm_roles):
        """A chain longer than the cap renders in full once the cap is raised (issue #43)."""
        topo = sf_mux_long_chain(wdm_site, dt_cwdm_sf, dt_pp, wdm_roles, passthrough_units=CHAIN_UNITS)
    ...
    def test_cap_hit_logs_warning(self, wdm_site, dt_cwdm_sf, dt_pp, wdm_roles, caplog):
        """Hitting the cap truncates the rendered segment and logs a warning (issue #43)."""
        topo = sf_mux_long_chain(wdm_site, dt_cwdm_sf, dt_pp, wdm_roles, passthrough_units=CHAIN_UNITS)
        far_rp = topo.bundles["mux_b"].line_ports["bidi"].rear_port
        with override_settings(PLUGINS_CONFIG=_plugins_config(CAP_BELOW_CHAIN)):
            with caplog.at_level(logging.WARNING, logger="netbox_wdm.core_walk"):
