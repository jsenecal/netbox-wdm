...
- pytest-xdist is now a dev dependency, enabling parallel local test runs (`pytest -n auto --dist loadscope`). pytest-django gives every xdist worker its own test database (suffixed `_gw0`, `_gw1`, ...), so workers are fully isolated at the database level. ([#48](https://github.com/jsenecal/netbox-wdm/issues/48))
- `max_trace_hops` plugin setting (default 100): the cable-chain walk cap used by wavelength path discovery and circuit trace rendering is now configurable via `PLUGINS_CONFIG`, counted in cable segments, and hitting the cap logs a warning instead of silently returning an incomplete path. ([#43](https://github.com/jsenecal/netbox-wdm/issues/43))
...
- CI additionally tests against NetBox 4.7.0, and the Codecov coverage
  ...
  resolve on 4.7.0. NetBox 4.7 is now a supported series: the last gap, the
  cassette overlay's handling of 4.7 module relocation, is fixed (see below).
...
- Unprofiled multi-terminated cables no longer have their strand pairing guessed from `CableTermination` row order. Core follows every fibre of such a cable at once, and the far line port is chosen by WDM role -- a TX origin lands on the far RX -- which reads the direction off the overlay instead of relying on row order that re-terminating can change. Profiled cables are unaffected: they resolve to a single strand. ([#49](https://github.com/jsenecal/netbox-wdm/issues/49))
- One node rebuild of a duplex CWDM mux pair now issues about 330 queries, up from about 150: core's walker goes further than the hand-rolled one it replaces and resolves cable profiles and positions on the way, and the loop pre-scan adds roughly 56. Walks are still memoized per rebuild pass, so cost does not scale with channel count. ([#49](https://github.com/jsenecal/netbox-wdm/issues/49))
...
- Test-suite performance: the read-only trace test classes (`tests/test_trace_duplex.py` and the builder-profile and unprofiled-fallback classes in `tests/test_cable_pairing.py`) now share one committed topology per test class instead of rebuilding it for every test, cutting several minutes from the suite without changing any assertion semantics. ([#48](https://github.com/jsenecal/netbox-wdm/issues/48))
- Wavelength path rebuilds now share a per-pass read cache (`trace.TraceCache`) across the traces of one `rebuild_wavelength_paths_for_node()` call. Each grid position used to re-walk the same trunk with fresh queries; the first walk now pays the cable-chain lookups and the remaining walks are served from memory, cutting one node rebuild of a duplex CWDM mux pair from 732 queries to 166. Trace semantics are unchanged and the cache never outlives a single rebuild pass. ([#48](https://github.com/jsenecal/netbox-wdm/issues/48))
...
- Relocating an installed module no longer strands its WDM overlay. NetBox 4.7 moves a module by saving the existing row into a different module bay, another device's included; the overlay treated a module as only ever installed or deleted, so its channels and line ports stayed on the WDM node the module left. `WdmChannel.clean()` and `WdmLinePort.clean()` then rejected every subsequent edit of those rows ("Module belongs to a different device than the WDM node"), and neither node had its expected port hash recomputed or its wavelength paths retraced. The rows now follow the module onto the node that owns its new device -- keeping each channel's status -- or are dropped when that device carries no WDM node, exactly as removing the module would; both ends are then pruned and retraced. ([#76](https://github.com/jsenecal/netbox-wdm/issues/76))
