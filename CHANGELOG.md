...
- Save-time cable validation: a `netbox.signals.post_clean` receiver on `dcim.Cable` now rejects role-incompatible WDM trunk terminations (TX-to-TX and RX-to-RX line port pairs) with a `ValidationError`, so the NetBox cable form and REST API refuse the miscable instead of accepting it and flagging the resulting wavelength path invalid after the fact. Duplex multi-terminated cables are checked per fibre index, matching the tracer's pairing convention; terminations that are not WDM line ports are never inspected. A bare `.save()` in a script still bypasses `clean()`, so the existing detection machinery remains as a backstop. ([#42](https://github.com/jsenecal/netbox-wdm/issues/42))
